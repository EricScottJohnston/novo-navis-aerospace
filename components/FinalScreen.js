...
// Lazy-loaded after the user answers question 3.
// Contains all pricing tiers, modals, and checkout logic.
// Props:
//   isEnterprise  — boolean, derived from round 3 answer
//   onRestart     — callback to reset back to question 1
...
export default function FinalScreen({ isEnterprise, onRestart }) {
  ...
  return (
    <>
      ...
      {TIERS.filter(t => isEnterprise ? t.key === 'enterprise' : t.key !== 'enterprise').map(({ key, name, price, badge }) => (
        <div key={key} style={{
          ...
          <button
            ...
            onMouseEnter={e => {
              if (!checkoutLoading && key === 'blueprint') {
                e.currentTarget.style.boxShadow = '0 6px 20px rgba(200,169,110,0.5)'
                e.currentTarget.style.transform = 'translateY(-1px)'
              }
            }}
            onMouseLeave={e => {
              if (key === 'blueprint') {
                e.currentTarget.style.boxShadow = '0 4px 14px rgba(200,169,110,0.35)'
      ...
      <p style={{ textAlign: 'center', color: '#8a95aa', fontSize: '0.82rem', margin: '0 0 1rem' }}>
        Questions?{' '}
        <button
          onClick={() => { setObjOpen(true); track('modal_opened', { modal: 'what_is_blueprint' }) }}
        ...
        {' '}or{' '}
        <button
          onClick={() => { setReviewsOpen(true); track('modal_opened', { modal: 'reviews' }) }}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#8a95aa', fontSize: '0.82rem', textDecoration: 'underline', textUnderlineOffset: '3px', padding: 0 }}
        >
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              <button
                ...
                style={{ width: '100%', padding: '0.95rem 1.25rem', background: NAVY, border: 'none', borderRadius: '10px', color: '#fff', fontWeight: 'bold', fontSize: '0.97rem', cursor: 'pointer' }}
              >
        ...
        >
          ...
          >
            <div style={{
              ...
              <button
                onClick={() => setSampleOpen(false)}
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#8a95aa', fontSize: '1.3rem', lineHeight: 1, padding: '0.2rem 0.4rem' }}
              >✕</button>
            </div>
            <iframe
      ...
      {objOpen && (
        ...
        >
          ...
          >
            ...
            <button
              onClick={() => setObjOpen(false)}
              style={{ width: '100%', padding: '0.85rem', background: NAVY, border: 'none', borderRadius: '8px', color: '#ffffff', fontWeight: 'bold', fontSize: '0.95rem', cursor: 'pointer' }}
            >
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              <button
                onClick={() => setShowTermsModal(false)}
                style={{ flex: 1, padding: '0.75rem', background: 'transparent', border: '1px solid #1e2a45', borderRadius: '6px', color: '#8a95aa', cursor: 'pointer', fontSize: '0.9rem' }}
              >Cancel</button>
              <button
